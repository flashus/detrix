//! Go validator unified tests
//!
//! Go has full feature parity with Python for safety validation.
//! Uses in-process tree-sitter parsing for AST analysis.
...
fn create_validator() -> GoValidator {
    GoValidator::with_defaults()
}

type TestData = GoTestData;
...
fn test_unified_strict_blocks_unknown() {
    let validator = create_validator();
    let result = validator
        .validate("customUnknownFunc(x)", SafetyLevel::Strict)
        .expect("Validation should succeed");
...
fn test_unified_trusted_allows_unknown() {
    let validator = create_validator();
    let result = validator
        .validate("customUnknownFunc(x)", SafetyLevel::Trusted)
        .expect("Validation should succeed");
